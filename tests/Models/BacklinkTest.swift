...
extension Tests.BacklinkTest {
	...
	func unlinkedReferenceCountMatchesGroupedQuery() throws {
		...
		let firstMention = Paragraph(string: "A plain Target Page mention", parentId: firstSource.id, pageId: firstSource.id, order: 0)
		let secondMention = Paragraph(string: "Another Target Page mention", parentId: secondSource.id, pageId: secondSource.id, order: 0)
		...
		expectNoDifference(Dictionary(uniqueKeysWithValues: groupedReferences.map { ($0.pageID, $0.referencedBlockIDs) }), [
			firstSource.id: [firstMention.id], secondSource.id: [secondMention.id],
		])
		#expect(count == 2)
