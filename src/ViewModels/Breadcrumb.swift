...
struct Breadcrumb: Identifiable {
	...
	var title: String? {
		canonicalTitle.map { Page.title(for: $0) }
	}

	var text: String {
		if let title { return title }
		if let string { return renderPlainText(fromMarkup: string) }

		fatalError("Blocks are either pages or paragraphs")
	}

	var page: Destination.Pages {
		switch (canonicalTitle, string) {
	...
	static func forBlock(id: Block.ID) -> some PartialSelectStatement<Breadcrumb> {
		let page = BlockHierarchy
			...
			.select { _, pages in
				PathRow.Columns(id: pages.id, title: pages.canonicalTitle.asOptional, string: String?.none, isPage: true, depth: 0)
			}
		...
		return With {
			...
			PathRow
				...
				.select { Breadcrumb.Columns(id: $0.id, canonicalTitle: $0.title, string: $0.string) }
		}
	}
}
